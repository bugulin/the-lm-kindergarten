"""
Evaluation kit for subtask 1.

The original code is available in the shared task's repository at
...
def calculate_smooth_combined_metric(
    overall_accuracy: float, total_content_effect: float
) -> float:
    """
...
def run_full_scoring(
    reference_data_path: str, prediction_path: str, output: Writer[str]
):
    """
    ...
    try:
        ...
        if diff != 0:
            print(
                "Error: not all the examples in ground truth have a corresponding prediction",
                file=sys.stderr,
            )
            final_results = {"accuracy": 0.0, "f1_premises": 0.0, "combined_score": 0.0}
            # --- Write Results ---
            try:
                json.dump(final_results, output)
        ...
        combined_smooth_score = calculate_smooth_combined_metric(
            overall_acc, tot_content_effect
        )
